...
/* From the Sparkfun schematic for the CAN-BUS shield, CS for the SD card is on B1 (digital pin 9) */
#define CS_LOW()		PORTB &= ~_BV(PORTB1)	/* Set MMC_CS = low */
#define	CS_HIGH()		PORTB |= _BV(PORTB1)	/* Set MMC_CS = high */
...
{
	CS_HIGH();		/* Set CS# high */
	spi_xchg(0x55);	/* Dummy clock (force DO hi-z for multiple slave SPI) */
}
...
)
{
	...
	spi_block_rx(buff, btr);		/* Receive the data block into buffer */
...
)
{
	...
	xprintf(PSTR("Waiting for card ready...\n"));
#endif
	if (!wait_ready(500)) return 0;		/* Leading busy check: Wait for card ready to accept data block */

#if defined (MMC_DEBUG)
	xprintf(PSTR("Sending data token...\n"));
#endif
	spi_xchg(token);					/* Xmit data token */
	if (token == 0xFD) return 1;		/* Do not send data if token is StopTran */

#if defined (MMC_DEBUG)
	xprintf(PSTR("Sending data...\n"));
#endif
	spi_block_tx(buff, 512);			/* Data */
	spi_xchg(0xFF); spi_xchg(0xFF);		/* Dummy CRC */

#if defined (MMC_DEBUG)
	xprintf(PSTR("Receiving response...\n"));
#endif
	resp = spi_xchg(0xFF);				/* Receive data resp */

#if defined (MMC_DEBUG)
	xprintf(PSTR("Response: 0x%X\n"), resp);
#endif
...
)
{
	...
	/* Send command packet */
#if defined (MMC_DEBUG)
	xprintf(PSTR("Sending command packet...\n"));
#endif
...
#if defined (MMC_DEBUG)
	xputs(PSTR("Power off complete. Continuing initialization...\n"));
#endif

	power_on();							/* Turn on the socket power */
	for (n = 10; n; n--) spi_xchg(0xFF);	/* 80 dummy clocks */
#if defined (MMC_DEBUG)
	xputs(PSTR("Power on complete. Continuing initialization...\n"));
#endif
...
#if defined (MMC_DEBUG)
		xputs(PSTR("CMD0 sent. Continuing initialization...\n"));
#endif
		Timer1 = 100;						/* Initialization timeout of 1000 msec */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* Is the card SDv2? */
#if defined (MMC_DEBUG)
			xputs(PSTR("SDv2 card detected. Continuing initialization...\n"));
#endif
...
#if defined (MMC_DEBUG)
				xputs(PSTR("SDv1 card detected. Continuing initialization...\n"));
#endif
				ty = CT_SD1; cmd = ACMD41;	/* SDv1 */
			} else {
#if defined (MMC_DEBUG)
				xputs(PSTR("MMCv3 card detected. Continuing initialization...\n"));
#endif
...
{
	if (!count) {
		xprintf(PSTR("About to return RES_PARERR!"));
	    return RES_PARERR;
	}
	if (Stat & STA_NOINIT) {
		xprintf(PSTR("About to return RES_NOTRDY!"));
		return RES_NOTRDY;
	}
	if (Stat & STA_PROTECT) {
		xprintf(PSTR("About to return RES_WRPRT!"));
		return RES_WRPRT;
	}
	...
	if (count == 1) {	/* Single block write */
		xprintf(PSTR("About to write a single block: 0x%lx\n"), sector);
		...
		xprintf(PSTR("About to write multiple blocks: %u 0x%lx\n"), count, sector);
